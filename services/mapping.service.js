...
const parseQuery = (query, origin) => {
  ...
  if (origin === 'gbif') {
    ...
    const hasGeo = (query.has || '').includes('geo')
    if (hasGeo) {
  ...
  if (query.scientificName && origin === 'natusfera') {
    query.scientificName = (query.scientificName || '').split(' ').slice(0, 2).join(' ')
  } else {
    query.scientificName = (query.scientificName || '')
  }
  if (origin === 'plantnet') {
  ...
  if (origin === 'odourcollect') {
    if (query.per_page || !query.per_page) {
      query.limitToLast = Number(query.per_page || 30)
      delete query.per_page
      delete query.limit
    }
    if (query.endAt) {
      query.endAt = query.endAt
      delete query.page
    }
    query.orderBy = '"created_at"'
  }
  if (query.quality_grade && query.quality_grade.includes('casual') && query.quality_grade.includes('research')) {
...
const generalMapping = item => {
  if (!item.measurements) {
    const user_id = item.user_id || (item.user ? (item.user.id || item.user.login || null) : null)
    ;(item.media || []).map(i => {
      const id = encrypt(item.id, i.large_url || i.identifier, {user_id})
      i.large_url = `https://us-central1-cos4cloud-2d9d3.cloudfunctions.net/proxy?id=${id}`
      return i
    })
  }
  return item
...
const basicMapping = item => {
  ...
  const {
    id, media, decimalLatitude, decimalLongitude
  } = item
  return {
    id, media, decimalLatitude, decimalLongitude
  }
...
module.exports = class MappingService {
  ...
  static async dwcGet(req, res) {
    ...
    const promises = origins.map(ogn => {
      if (originsConfig[ogn]) {
        ...
        if (ogn === 'canairio' && req.query.page) {
          return []
        }
        ...
        .then(res => {
          ...
          if (originConfig.filter) {
            return Object.values(res).filter(i => originConfig.filter(i, query)).map(mapp)
          }
          if (res && res.results) return res.results.map(mapp)
          if (res && res.observations) return res.observations.map(mapp)
          if (res && !res.splice) return Object.values(res).filter(Boolean).map(mapp)
          else return res && res.map ? res.map(mapp) : []
        })
        .then(res => map ? res.map(basicMapping) : res.map(generalMapping))
      }
      return []
    })
  ...
  static async dwcGetById(req, res) {
    ...
    if (originsConfig[ogn]) {
      ...
      return fetch(`${originConfig.url}${path}`, originHeader)
      .then(res => res.json())
      .then(res => {
        const mapp = originConfig.mapping || ((i) => i)
        return generalMapping(mapp(res))
      })
    }
    return {}
  }

  static async _dwcGetById(id, resource = 'natusfera') {
    const ogn = resource
