...
const update = async (ref, data) => {
  ...
  if (id) {
    ...
    const aux = await db.collection(ref).findOneAndUpdate({ _id: id }, { $set: { ...data }, $setOnInsert: { created_at: new Date() } }, { upsert: true })
    return aux.value    
  } else {
    delete data.created_at
...
exports.logs = async (ref, params) => {
  const db = await connectToDatabase();

  const { origin, ref_id, ref_user } = params
  const queryParams = {ref_user, ...(origin ? { origin, ref_id } : { ref_id })}
  const query = JSON.parse(JSON.stringify(queryParams))

  const aux = {}
  const promises = []
  promises.push(db.collection(ref).aggregate([
    { $match: query },
    { $unwind: { path: '$ref_id', preserveNullAndEmptyArrays: true } },
  ...
  promises.push(db.collection(ref).aggregate([
    { $match: query },
    { $unwind: { path: '$browser.name', preserveNullAndEmptyArrays: true } },
    { $group: { _id: '$browser.name', count: { $sum: 1 } } }
  ]).toArray())
  promises.push(db.collection(ref).aggregate([
    { $match: query },
    { $unwind: { path: '$os.name', preserveNullAndEmptyArrays: true } },
  ...
  promises.push(db.collection(ref).aggregate([
    { $match: query },
    { $unwind: { path: '$ip', preserveNullAndEmptyArrays: true } },
  ...
  const [ countAgg, countryAgg, browserAgg, osAgg, hostAgg, ipAgg, langAgg, originsAgg ] = await Promise.all(promises)

  aux.count = countAgg
  aux.countries = countryAgg
  aux.browser = browserAgg
  aux.os = osAgg
  aux.host = hostAgg
  aux.ip = ipAgg
  aux.lang = langAgg
